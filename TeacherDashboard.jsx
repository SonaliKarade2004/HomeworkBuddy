...
import { FaTrash, FaStar, FaUsers, FaCommentDots, FaBell } from "react-icons/fa";
import { MdAssignmentAdd } from "react-icons/md";
import backteach from "./assets/teacher-back.jpg";
...
const TeacherDashboard = () => {
  ...
  const addStudent = async () => {
    ...
    const res = await teacherApi.createStudent({
      name: newStudent,
      email: `${newStudent.toLowerCase().replace(" ", "")}@school.com`,
      class: "Grade 10",
    });
  ...
  formData.append("title", newHomework);
  ...
  body: formData,
    ...
    // Update frontend UI
    setAssignedHomework((prev) => [...prev, data]);
    setNewHomework("");
    setDueDate("");
  ...
  const missingSubmissions = students.filter((s) => s.homeworks.some((hw) => hw.badges === 0));

  return (
    <div
      className="teacher-dashboard section-bg"
      style={{
...
{activeTab === "homework" && (
  ...
  />
  <input
  key={selectedFile ? selectedFile.name : "file"} // forces input to reset
  type="file"
        ...
        <ul>
          {assignedHomework.map((hw, i) => (
            <li key={i}>
              ...
              {hw.fileName && <span>📎 {hw.fileName}</span>}
            </li>
          ))}
        </ul>
      </div>
      ...
      {activeTab === "notifications" && (
        <div className="notifications-section fade-in">
          ...
          {missingSubmissions.length === 0 ? (
            <div className="notif-card success">🎉 All students have completed feedbacks!</div>
          ) : (
